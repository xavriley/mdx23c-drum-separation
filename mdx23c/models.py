...
class _TFC_TDF(nn.Module):
    """TFC-TDF block implementation."""
...
class TFC_TDF_net(nn.Module):
    """Main TFC-TDF network for source separation."""
    
    def __init__(self, config):
        super().__init__()
        self.config = config

        norm = _get_norm_factory(norm_type=config.model.norm)
        ...
        for _ in range(n):
            block = nn.Module()
            block.tfc_tdf = _TFC_TDF(c, c, l, f, bn, norm, act)
            block.downscale = _Downscale(c, c + g, scale, norm, act)
            f = f // scale[1]
            c += g
            self.encoder_blocks.append(block)

        self.bottleneck_block = _TFC_TDF(c, c, l, f, bn, norm, act)

        self.decoder_blocks = nn.ModuleList()
        for _ in range(n):
            block = nn.Module()
            block.upscale = _Upscale(c, c - g, scale, norm, act)
            f = f * scale[1]
            c -= g
            block.tfc_tdf = _TFC_TDF(2 * c, c, l, f, bn, norm, act)
        ...
        self.stft = _STFT(config.audio)

    def cac2cws(self, x):
        """Convert from channel-audio-channel to channel-window-subband format."""
        k = self.num_subbands
        b, c, f, t = x.shape
    ...
    def cws2cac(self, x):
        """Convert from channel-window-subband to channel-audio-channel format."""
        k = self.num_subbands
        b, c, f, t = x.shape
